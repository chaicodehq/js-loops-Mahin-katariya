...
export function cricketScoreboard(balls) {
  ...
  if(!Array.isArray(balls) || balls.length == 0) return {totalRuns, totalBalls, wickets, fours, sixes};
  for(let i=0; i< balls.length; ++i){
    switch(balls[i]){
      ...
      case -1:
        totalBalls += 1;
        wickets += 1;
        if(wickets == 10) return {totalBalls, totalRuns, sixes, fours, wickets};
        break;
      default:
        totalBalls += 1;
        totalRuns += balls[i];
  ...
  return {totalRuns,totalBalls,sixes,fours,wickets}
}
